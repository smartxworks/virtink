...
func main() {
	...
	cloudHypervisorCmd := []string{"cloud-hypervisor", "--api-socket", "/var/run/kubrid/ch.sock", "--console", "pty", "--serial", "tty"}
...
func buildVMConfig(ctx context.Context, vm *kubridv1alpha1.VirtualMachine) (*cloudhypervisor.VmConfig, error) {
	...
	if vm.Spec.Instance.Kernel != nil {
		vmConfig.Kernel.Path = "/mnt/kubrid-kernel/vmlinux"
...
func startDHCPServer(ifaceName string, mac net.HardwareAddr, ipNet *net.IPNet, gateway net.IP) error {
	...
	dnsmasqPIDPath := fmt.Sprintf("/var/run/kubrid/dnsmasq/%s.pid", ifaceName)
	if err := os.MkdirAll(filepath.Dir(dnsmasqPIDPath), 0755); err != nil {
		return fmt.Errorf("create dnsmasq PID dir: %s", err)
	}

	dnsmasqConfPath := fmt.Sprintf("/var/run/kubrid/dnsmasq/%s.conf", ifaceName)
